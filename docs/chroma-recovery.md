...
Figure 1 — Chroma recovery overview (Candy Candy comparison).

Read [start-here.md](start-here.md) first if you are new to the repo. That page covers the shared workflow through source prep, dataset curation, alignment, and the branch decision. This page picks up at the chroma branch and focuses on chroma-specific target construction, training considerations, and validation.
...
Recover believable color from a faded source using a stronger reference while preserving original source detail.

Follow the stages in order and treat each section as a checklist.
...
- Use this page when color loss is the main problem
